...
try:
    ...
    raise MissingDependency('the whoosh backend requires the installation of whoosh. Please refer to the documention.')
...
class WhooshHtmlFormatter(HtmlFormatter):
    ...
    def __init__(self, connection_alias, **connection_options):
        ...
        """
        super(WhooshHtmlFormatter, self).__init__(connection_alias, **connection_options)
    ...
    def setup(self):
        ...
        if self.use_file_storage and not os.access(self.path, os.W_OK):
            raise IOError("The path to you Whoosh index '%s' is not writable for the current user/group." % self.path)
        ...
        self.content_field_name, self.schema = self.build_schema(
            connections[self.connection_alias].get_unfied_index().all_searchfields())
    ...
    def remove(self, obj_or_string, commit=True):
        ...
        try:
            self.index.delete_by_query(q=self.parse.parse(u'%s: "%s"' % (ID, whoosh_id)))
        except Exception as e:
            if not self.silently_fail:
                raise
    ...
    def clear(self, models=None, commit=True):
        ...
        try:
            if models is None:
                ...
                self.index.delete_by_query(q=self.parse.parse(u'OR'.join(models_to_delete)))
            ...
            if models is not None:
                self.log.error('Failed to clear whoosh index of models "%s": %s', ','.join(models_to_delete), e,
                               exc_info=True)
            else:
    ...
    def search(self, query_string, sort_by=None, start_offset=0, end_offset=None, fields='', highlight=False,
               facets=None, date_facets=None, query_facets=None, narrow_queries=None, spelling_query=None, within=None,
               dwithin=None, distance_point=None, models=None, limit_to_registered_models=None, result_class=None,
               **kwargs):
        if not self.setup_complete:
            self.setup()
        ...
        if date_facets is not None:
            warnings.warn('whoosh does nnot handle date faceting.', Warning, stacklevel=2)
        ...
        if narrow_queries is not None:
            ...
            for nq in narrow_queries:
                recent_narrowed_results = narrow_searcher.search(self.parser.parse(force_text(nq)), limit=None)
    ...
    def more_like_this(self, model_instance, additional_query_string=None, start_offset=0, end_offset=None,
        ...
        if not self.setup_complete:
            ...
            if narrow_queries is not None:
                narrow_searcher = self.index.searcher()
                for nq in narrow_queries:
                    recent_narrowed_results = narrow_searcher(self.parse.parse(force_text(nq)), limit=None)
                    if len(recent_narrowed_results) <= 0:
                        return {
                            'results': [],
                            'hits': 0,
