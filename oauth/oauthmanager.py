# -*- coding: utf-8 -*-
# @Author:  ty
# @FileName: oauthmanager.py
# @Time:  2021/2/21 下午11:52
# @Description:
import json
import logging
import urllib
...
class BaseOauthManager(metaclass=ABCMeta):
    """获取用户授权"""
    AUTH_URL = None
    TOKEN_URL = None  # 用户token
    API_URL = None  # 用户信息
    ICON_NAME = None  # icon图标名
...
class WBOauthManager(BaseOauthManager):
    ...
    def get_authorization_url(self, next_url='/'):
        params = {
            ...
            'response_type': 'code',
            'redirect_url': self.callback_url + '&next_url=' + next_url
        }
    ...
    def get_access_token_by_code(self, code):
        ...
        if 'access_token' in obj:
            ...
            raise OAuthAccessTokenException(resp)
...
class GoogleOauthManager(BaseOauthManager):
    """谷歌授权"""
    AUTH_URL = 'https://accoounts.google.com/o/oauth2/v2/auth'
    ...
    def get_access_token_by_code(self, code):
        ...
        if 'access_token' in obj:
            ...
            logger.info(self.ICON_NAME + ' oauth ' + resp)
    ...
    def get_oauth_userinfo(self):
        ...
        try:
            ...
            logger.error(e)
            logger.error('google oauth error.rsp:' + resp)
            return None
...
class GitHubOauthManager(BaseOauthManager):
    ...
    def get_oauth_userinfo(self):
        ...
        try:
            ...
            logger.error(e)
            logger.error('github oauth error.rsp:' + resp)
            return None
...
class FacebookOauthManager(BaseOauthManager):
    ...
    def __init__(self, access_token=None, openid=None):
        ...
        self.client_secret = config.appsecret if config else ''
        self.callback_url = config.callback_url if config else ''
        super(FacebookOauthManager, self).__init__(access_token=access_token, openid=open)
...
class QQOauthManager(BaseOauthManager):
    ...
    def get_access_token_by_code(self, code):
        ...
        resp = self.do_get(self.TOKEN_URL, params)
        if resp:
            res = urllib.parse.parse_qs(resp)
...
def get_oauth_apps():
    ...
    apps = [x() for x in applications if x().ICON_NAME.lower() in config_types]
    return apps
